...
JeanContext.prototype.scan = function(path, callback) {
    ...
    if (!path) {
        return callback(new Error('You should provide path as an argument or in config.scan.path variable'));
    }
    ...
    recursive(path, self.config.scan.exclude, function (err, files) {
        ...
        var processFile = function(file) {
            ...
            try {
                var m = require(file);
                if (m.$bean) {
                    self.register(m.$bean, m, function(err) {
                        if (err) {
                            return callback(err);
                        }
                        processFile(files[i++]);
...
JeanContext.prototype.register = function(beanName, beanClass, callback) {
    ...
    if (!beanName) {
        return callback(new Error('No name provided for bean ' + beanClass));
    }

    if (self.beans[beanName]) {
        return callback(new Error(beaname + ' already in context'));
    }
